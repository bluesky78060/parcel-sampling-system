...
/**
 * 경영체번호 기반 보조 키. PNU 표기가 파일마다 달라 `parcelMatchKey`가 빗나갈 때 쓴다.
 *
 ...
 *
 * **경영체번호가 비면 키를 만들지 않는다.** 대표필지 파일에 번호가 없을 수 있는데,
 * 빈 값끼리 매칭시키면 무관한 필지가 같은 필지가 된다.
 *
 * **지번이 비어도 키를 만들지 않는다**(PROJ1-1-40). `F001_A리_` 형태가 되어 한 농가가
 * 같은 리에 가진 지번 미상 필지들이 전부 같은 필지가 된다 — 형제 함수 `parcelMatchKey`가
 * `'__'`를 돌려주던 것을 `null`로 바꾼 것과 정확히 같은 이유인데(PROJ1-1-37) 이쪽만
 * 방치돼 있었다. 폴백이 통째로 없어지는 것이 아니다: **지번이 있는 필지는 그대로
 * 매칭된다.** 지번도 PNU도 주소도 없는 행은 애초에 어느 필지인지 말할 수 없다.
 *
