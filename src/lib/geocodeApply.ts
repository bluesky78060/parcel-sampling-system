 ...
 *
 * ⚠️ **단, 실패가 `quota`·`unreachable`·`auth`면 지우는 것이 맞지 않다.** 그것들은
 * 데이터가 아니라 서버 사정이라 좌표 유무에 대해 아무 말도 하지 않는다. 실측:
 * 좌표를 다 가진 200건에서 서버 무응답 시 150건, `notFound` 폭주 시 **0건**이 되고
 * 화면은 "변환 완료"라고 말한다(캐시는 이미 비워진 뒤라 복구 불가).
 * **이 함수가 고칠 문제가 아니다** — 실패 종류를 아는 것은 `batchGeocoder`와
 * 호출부다. PROJ1-1-43으로 분리했다.
 *
