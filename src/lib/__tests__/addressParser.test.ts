...
describe('parseSido', () => {
  ...
  /**
   * **문서-구현 불일치.** `addressParser.ts:33`의 JSDoc이 `"충남 아산시…" → "충남"`
   * 이라고 계약을 명시했는데 구현은 빈 문자열을 돌려준다 — `[가-힣]+` 뒤에 접미사
   * (도/시/특별시/광역시/…)가 와야 하는데 "충남"의 '남'이 그 목록에 없다.
   *
   * 아래는 **JSDoc이 약속한 동작**을 기대값으로 적었다. 지금은 실패하므로
   * `it.fails`다. 고치면 이 테스트가 "예상과 달리 통과했다"며 실패하고,
   * 그때 `.fails`만 떼면 된다 — 현재 동작을 박제하면 고칠 때 기대값을
   * 다시 정해야 하고, 그 순간 "그때 나온 값"을 적어 넣는 사고가 난다.
   *
   * 봉화군 데이터는 "경상북도"라 드러나지 않는다. 다른 시군 파일이 축약형으로
   * 들어오면 시도가 통째로 비고 조립 주소에서도 빠진다. PROJ1-1-6에서 다룬다.
   */
  it.fails('[PROJ1-1-6 미수정] 축약형 시도명도 인식해야 한다 (JSDoc이 약속한 동작)', () => {
    expect(parseSido('충남 아산시 염치읍 강청리 123-4')).toBe('충남');
    expect(parseSido('경북 봉화군 봉화읍 거촌리 127번지')).toBe('경북');
  });
});
...
describe('normalizeAddress / isSameAddress', () => {
   ...
   *
   * **PNU가 있어도 안전하지 않다.** `duplicateDetector`의 `isMatched`는 PNU 매칭에
   * 실패하면 early return 없이 주소로 다시 시도한다(`duplicateDetector.ts:34-38`).
   * 따라서 이 충돌은 PNU 보유 필지에도 적용되고, 마스터 필지가 잘못 "기채취"로
   * 판정돼 추출 후보에서 조용히 빠질 수 있다.
   * 같은 동작을 `duplicateDetector.test.ts`의
   * "PNU가 달라도 주소가 같으면 중복으로 본다"가 직접 검증한다.
   */
