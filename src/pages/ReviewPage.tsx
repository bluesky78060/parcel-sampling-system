...
export function ReviewPage() {
  ...
  const allParcelsWithRep = useMemo(() => {
    ...
    // 키가 없는 대표필지는 마스터와 같은 필지인지 판정할 수 없다 — 새 것으로 본다.
  ...
  const mapSelectedParcels = useMemo(() => {
    // 정규 키를 쓴다. 예전 `farmerId__parcelId`는 경영체번호가 비면 리를 넘어 충돌해
    // 선택되지 않은 필지가 선택 마커로 찍혔다.
    //
    // 충돌 봉쇄는 유지한다 — PNU가 없고 주소가 같은 두 필지는 정규 키로도 겹칠 수 있다.
  ...
  const mapLegendCounts = useMemo(() => {
    ...
    for (const p of allParcels) {
      const key = parcelMatchKey(p);
      if (key !== null && repKeys.has(key)) continue; // 대표필지는 위에서 처리
      if (!p.coords) {
        noCoords++;
